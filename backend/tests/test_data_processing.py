...
class DataProcessorTests(unittest.TestCase):
    ...
    def test_processing_continues_with_next_pdf_after_three_failures(self):
        ...
        self.assertEqual(
            requested_paths,
            [failed_document, failed_document, failed_document, next_document],
        )
        self.assertEqual(company_df.iloc[0]["revenue"], 120)
        self.assertTrue(sens_df.empty)

    def test_newest_non_empty_value_wins_for_each_company_field(self):
        records = [
            {
                "company": "Example Company",
                "report_date": "2024-12-31",
        ...
        merged = Data_Processor._keep_most_recent_company_fields(records)

        self.assertEqual(len(merged), 1)
        self.assertEqual(merged[0]["company"], "example company")
        self.assertEqual(merged[0]["report_date"], "2025-12-31")
        self.assertEqual(merged[0]["revenue"], 120)
        self.assertEqual(merged[0]["employee_count"], 50)
        self.assertEqual(
            merged[0]["countries_of_operation"], ["South Africa"]
        )
    ...
    def test_yfinance_fills_missing_values_and_replaces_mismatches(self):
        ...
        with patch(
            ...
            validated_sens, validated_expenditure = (
                processor.validate_external_data(sens_df, expenditure_df)
            )

        pd.testing.assert_frame_equal(validated_sens, sens_df)
        yfinance_ticker.assert_called_once_with("GLN.JO")
        self.assertEqual(validated_expenditure.iloc[0]["revenue"], 100)
        self.assertEqual(
            validated_expenditure.iloc[0]["original_currency"], "USD"
        )
        self.assertEqual(validated_expenditure.iloc[0]["cost_of_sales"], 40)
        self.assertEqual(validated_expenditure.iloc[0]["total_debt"], 25)
        self.assertEqual(
            validated_expenditure.iloc[0]["capital_expenditure"], 6
        )
        self.assertEqual(
    ...
    def test_extraction_normalizes_gemini_output_to_stable_codes(self):
        ...
        self.assertEqual(external_df.iloc[0]["reporting_unit"], "millions")
        self.assertEqual(external_df.iloc[0]["revenue"], 1250.0)
        self.assertEqual(
        ...
        self.assertEqual(sens_df.iloc[0]["event_unit"], "thousands")
        self.assertEqual(sens_df.iloc[0]["currency"], "GBP")
        self.assertEqual(sens_df.iloc[0]["country"], "GB")

    def test_standardize_data_scales_and_converts_both_frames_to_zar(self):
        processor = Data_Processor()
        sens_df = pd.DataFrame([{
            "company": "Glencore plc",
            "announcement_date": "2025-01-10",
            "event_value": 4,
            "event_unit": "thousands",
            "currency": "GBP",
            "country": "United Kingdom",
        ...
        self.assertEqual(external_row["company"], "Glencore")
        ...
        self.assertEqual(external_row["countries_of_operation"], ["ZA"])
        self.assertEqual(external_row["currencies_exposed_to"], ["USD", "ZAR"])
        ...
        self.assertEqual(sens_row["company"], "Glencore")
        ...
        self.assertEqual(sens_row["country"], "GB")
        ...
        self.assertEqual(external_df.iloc[0]["revenue"], 2)
        self.assertEqual(sens_df.iloc[0]["event_value"], 4)

    def test_standardize_data_scales_zar_without_calling_yfinance(self):
        processor = Data_Processor()
        sens_df = pd.DataFrame([{
            "announcement_date": "2025-01-10",
            "event_value": 2,
    ...
    def test_validation_keeps_standardized_values_in_zar(self):
        ...
        with patch(
            ...
            validated_sens, validated_external = (
